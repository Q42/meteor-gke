...
ddpclient.connect(function(error, wasReconnect) {
   ...
   */
  ddpclient.subscribe(
    "clicks",                  // name of Meteor Publish function to subscribe to
    [],                       // any parameters used by the Publish function
    function () {             // callback when the subscription is complete
      console.log("clicks complete:");
      console.log(ddpclient.collections.clicks);
    }
  );

  setInterval(function () {
    ddpclient.call(
      "/clicks/insert",             // name of Meteor Method being called
      [{click: new Date()}],            // parameters to send to Meteor Method
      function (err, result) {   // callback which returns the method call results
        console.log("called function, result: " + result);
      },
      function () {              // callback which fires when server has finished
        console.log("updated");  // sending any updated documents as a result of
        //console.log(ddpclient.collections.clicks);  // calling this method
      }
    );
  }, 1000);
   ...
   */
  var observer = ddpclient.observe("clicks");
